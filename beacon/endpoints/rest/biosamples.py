...
def build_response_individuals(rows, qparams_db, non_accessible_datasets):
    return build_beacon_response(rows,
                                 ...
                                 build_biosample_or_individual_response,
                                 biosample_stable_id=qparams_db.targetIdReq)
...
handler_individuals = generic_handler('individuals', individuals_proxy, db.fetch_individuals_by_biosample, build_biosample_or_individual_response)
handler_biosamples  = generic_handler('biosamples' , biosamples_proxy , db.fetch_biosamples_by_biosample , build_biosample_or_individual_response)
handler_gvariants   = generic_handler('gvariants'  , gvariants_proxy  , db.fetch_variants_by_biosample   , build_variant_response)
