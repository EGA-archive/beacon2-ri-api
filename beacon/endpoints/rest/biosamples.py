...
def fetch_individuals(qparams_db, datasets, authenticated):
    return db.fetch_individuals(qparams_db, datasets, authenticated, biosample_stable_id=qparams_db.targetIdReq)

def fetch_biosamples(qparams_db, datasets, authenticated):
    return db.fetch_biosamples(qparams_db, datasets, authenticated, biosample_stable_id=qparams_db.targetIdReq)

def fetch_variants(qparams_db, datasets, authenticated):
    return db.fetch_variants(qparams_db, datasets, authenticated, biosample_stable_id=qparams_db.targetIdReq)
