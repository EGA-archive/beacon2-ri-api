-- FUNCTION: public.query_data_response(text, integer, integer, integer, integer, integer, integer, character varying, text, text, text, text, text)

-- DROP FUNCTION public.query_data_response(text, integer, integer, integer, integer, integer, integer, character varying, text, text, text, text, text);

CREATE OR REPLACE FUNCTION public.query_data_response(
	_variant_type text,
	_start integer,
	_start_min integer,
	_start_max integer,
	...
	_dataset_ids text,
	_filters text)
    RETURNS TABLE(id text, variant_composite_id text, chromosome text, variant_id text, reference text, alternate text, start integer, "end" integer, variant_type text, variant_cnt integer, call_cnt integer, sample_cnt integer, matching_sample_cnt integer, frequency numeric, dataset_id integer) 
    LANGUAGE 'plpgsql'
